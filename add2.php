...
$dbname = "elysian_interiors";
    ...
    // to test the code this is an example $id = 100000003; 
...
if (mysqli_connect_error()) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
        ...
        $category = $_POST['design-category'];
        $description = $_POST['project-description'];

        $imgName = 'default_Project.png'; // Default image name in case file upload fails
        if (isset($_FILES['project-image']) && $_FILES['project-image']['error'] == 0) {
            ...
            $time = time();
            $imgName = $time . "_" . basename($_FILES["project-image"]["name"]);
            $file = $folder . $imgName;
        ...
        $stmt = $connection->prepare("INSERT INTO designportoflioproject (designerID, projectName, projectImgFileName, description, designCategoryID) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssi", $id, $name, $imgName, $description, $categoryID);
        ...
        if ($stmt->execute()) {
            ...
            echo "Error in adding the project";
        }
    }
}
?>
